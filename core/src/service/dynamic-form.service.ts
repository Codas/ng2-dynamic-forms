...
import { DYNAMIC_FORM_CONTROL_TYPE_TYPEAHEAD, DynamicTypeaheadModel } from "../model/typeahead/dynamic-typeahead.model";

@Injectable()
export class DynamicFormService {

  constructor(@Inject(FormBuilder) private formBuilder: FormBuilder,
              @Inject(DynamicFormValidationService) private validationService: DynamicFormValidationService) {
  }
  ...
  createFormGroup(groupModel: DynamicFormControlModel[],
                  ...
                  parent?: DynamicPathable): FormGroup {
    ...
    groupModel.forEach(model => {
      ...
      if (model.type === DYNAMIC_FORM_CONTROL_TYPE_ARRAY) {
        ...
        const formGroupModel = model as DynamicFormGroupModel,
        ...
        const formControlModel = model as DynamicFormValueControlModel<DynamicFormControlValue>;
  ...
  getPathSegment(model: DynamicPathable): string {
    ...
    return model instanceof DynamicFormArrayGroupModel && model.index ?
  ...
  addFormGroupControl(formGroup: FormGroup,
    ...
    if (groupModel instanceof DynamicFormGroupModel) {
      ...
      const formModel = groupModel as DynamicFormControlModel[];
  ...
  insertFormGroupControl(index: number,
    ...
    const parent = groupModel instanceof DynamicFormGroupModel ? groupModel : undefined,
  ...
  findById(id: string, groupModel: DynamicFormControlModel[]): DynamicFormControlModel | null {
    ...
    let result: DynamicFormControlModel | null = null;
    ...
    raw.forEach((model: any) => {
      ...
      switch (model.type) {
        ...
        case DYNAMIC_FORM_CONTROL_TYPE_TYPEAHEAD:
          group.push(new DynamicTypeaheadModel(model, model.cls));
          break;
